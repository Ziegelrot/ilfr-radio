...
for (let i = 0, length = GetNumResourceMetadata("ilfr-radio", "ilfr_radio_file"); i < length; i++) {
    const radio = GetResourceMetadata("ilfr-radio", "ilfr_radio_file", i);
    ...
    try {
        const data = JSON.parse(GetResourceMetadata("ilfr-radio", "ilfr_radio_file_extra", i));
        if (data !== null) {
            customRadios.push({
                "isPlaying": false,
                "name": radio,
